...
const demo = src.replace(SEED_TAG, tag([
  ...
  '',
  '  /* ---- 直接跳到結局 ----',
  '     用老師身分把四層認列＋放行結局，再換回學生。走的是真的 API。 */',
  '  function jumpToEnd(btn) {',
  '    if (btn) { btn.textContent = \'跳過去⋯\'; btn.disabled = true; }',
  '    var teamId = null;',
  '    run(\'apiLogin\', { account: \'stu01\', password: \'pw1234\' })',
  '      .then(function (r) { return run(\'apiBootstrap\', r.token); })',
  '      .then(function (b) {',
  '        teamId = b && (b.myTeamId || b.teamId);',
  '        if (!teamId) throw new Error(\'no team\');',
  '        return run(\'apiLogin\', { account: \'tea01\', password: \'pw1234\' });',
  '      })',
  '      .then(function (r) {',
  '        var tk = r.token;',
  '        return run(\'apiSetTeamLayer\', tk, teamId, 4, \'試用：直接看最後狀態\')',
  '          .then(function () {',
  '            return run(\'apiOpenFinale\', tk, teamId,',
  '              \'四層都走完了。回頭把這一學期寫下來——這一份不評分。\');',
  '          });',
  '      })',
  '      .then(function () {',
  '        try { sessionStorage.setItem(FIN, \'1\'); } catch (e) {}',
  '        enter(\'stu01\');',
  '      })',
  '      .catch(function () {',
  '        if (btn) { btn.textContent = \'看結局\'; btn.disabled = false; }',
  '        window.alert(\'跳不過去。先讓試用班有資料（按重來重新灌一次）再試。\');',
  '      });',
  '  }',
  '',
  ...
  '  /* ---- 工具列 ---- */',
  '  function lockBar(on) {',
  '    var box = document.getElementById("jlz-trial");',
  '    if (!box) return;',
  ...
  '',
  '    mk(\'看結局\', \'#C77BE0\', function (e) {',
  '      if (!window.confirm(\'把這一組直接算成四層都走完、並放行結局？\\n\\n這是試用資料，按「重來」可以清掉。\')) return;',
  '      jumpToEnd(e && e.currentTarget);',
  '    }, \'四層直接認列＋放行結局。要看最後那一段互動用這個。\');',
  '',
  ...
  '    }',
  '    bar();',
  '    /* 資料還在灌的時候先鎖住工具列——這時候按「看結局」會因為還沒有',
  '       隊伍而失敗，只跳出一句「先按重來」，看起來像壞掉。 */',
  '    if (!get(TOK)) lockBar(true);',
  '    /* 剛剛按過看結局：等 app 起來，直接開結局那一頁 */',
  '    try {',
  ...
  '        var go = function () {',
  '          var A = window.App;',
  '          if (A && A.logic && A.logic.state && A.logic.state.signedIn) {',
  '            A.logic.setState({ screen: \'FINALE\' });',
  '            return;',
  '          }',
  '          if (++n < 100) setTimeout(go, 120);',
  '        };',
  '        setTimeout(go, 400);',
  '      }',
  '    } catch (e) {}',
  ...
  '    };',
  '    if (seeded()) first();',
  '    else SEED().then(first).catch(function () { lockBar(false); });',
  '  })();',
  '})();'
].join('\n')));
